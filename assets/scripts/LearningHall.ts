...
export class LearningHall extends Component {
  ...
  private drawTopBar(root: Node, t: ReturnType<LearningHall['theme']>) {
    ...
    this.drawRankBadge(root, rankIdx, textStart + 54, topY - 18, t);
  ...
  private renderRanks() {
    ...
    RANKS.forEach((rank, i) => {
      ...
      this.label(root, `HallRankRowName-${i}`, rank.name, -168, y + 9, 220, 24, 15, nameColor, 'left', 6);
      const req = (i === RANKS.length - 1) ? '150字全收集 + 经验12000' : `累计经验 ${rank.threshold}`;
      this.label(root, `HallRankRowReq-${i}`, req, -168, y - 11, 240, 20, 10, t.night ? new Color(216, 200, 168) : new Color(106, 74, 42), 'left', 6);
      ...
      this.label(root, `HallRankRowState-${i}`, state, 200, y, 90, 24, 12, stateColor, 'center', 6);
    });
  ...
  private onTouchStart(event: EventTouch) {
    ...
    if (this.mode === 'home') {
      ...
      else if (this.hit(x, y, -43, -281, 60, 60)) this.render('codex');
      else if (this.hit(x, y, 43, -281, 60, 60)) this.render('parent');
      else if (this.hit(x, y, 130, -281, 60, 60)) this.render('progress');
      else if (this.hit(x, y, 216, -281, 60, 60)) this.render('settings');
      return;
    }
    if (this.mode === 'ranks') {
      if (this.hit(x, y, 0, -230, 220, 50)) this.render('home');
      return;
    }
